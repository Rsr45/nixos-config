...
Process {
    ...
    command: [
        "sh", "-c",
        "dysk -u binary -f 'disk <> remov' -j | jq -r '.[0].stats.used, .[0].stats.available, .[0].stats.size, .[0].stats.\"used-percent\"'"
    ]
    ...
    stdout: StdioCollector {
        onStreamFinished: {
            ...
            root.storageUsed = toNum(parts[0])
            root.storageFree = toNum(parts[1])
            root.storageTotal = toNum(parts[2])
        root.storagePerc = parseFloat(parts[3]) / 100
        }
    }
}
    // Process {
